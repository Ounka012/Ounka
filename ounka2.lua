-- 🎵 Full Client Boombox System for Delta Executor
-- ដំណើរការភ្លាមៗ បង្ហាញ GUI លើអេក្រង់ មានបញ្ជីចម្រៀង បញ្ជាកម្រិតសំឡេង និងច្រើនទៀត
...
-- បង្កើត Sound Object (លាក់)
local Music = Instance.new("Sound")
Music.Volume = currentVolume
Music.Looped = false -- យើងនឹងគ្រប់គ្រងការចាក់បន្ទាប់ដោយខ្លួនឯង
Music.Parent = PlayerGui -- លាក់ក្នុង PlayerGui កុំឲ្យឃើញ
...
ScreenGui.ResetOnSpawn = false -- កុំឲ្យបាត់ពេលស្លាប់
...
MainFrame.Draggable = true -- អាចអូសបានភ្លាមៗ
...
local VolumeSlider = Instance.new("TextButton") -- ប្រើ TextButton ជា Slider
...
local function createButton(text, positionX, color, callback)
	local btn = Instance.new("TextButton")
	btn.Size = UDim2.new(0, 40, 0, 40)
	btn.Position = UDim2.new(0, positionX, 0, 5)
...
createButton("⏮", 100, Color3.fromRGB(100, 100, 100), function()
	if #SongLibrary <= 1 then return end
	Music:Stop()
...
ShuffleBtn.MouseButton1Click:Connect(function()
	...
	if isShuffle then
		ShuffleBtn.BackgroundColor3 = Color3.fromRGB(0, 120, 255)
	else
		ShuffleBtn.BackgroundColor3 = Color3.fromRGB(80, 80, 80)
	end
end)
...
PlaylistFrame.CanvasSize = UDim2.new(0,0,0,0) -- នឹងកែតម្រូវ
...
local function updatePlaylist()
	-- សម្អាតកូនចាស់ៗ (លើកលែង UIListLayout)
	...
	for idx, song in ipairs(SongLibrary) do
		...
		songBtn.MouseButton1Click:Connect(function()
			...
			updatePlaylist() -- ធ្វើបច្ចុប្បន្នភាពពណ៌
		end)
	end
...
-- ការគ្រប់គ្រងកម្រិតសំឡេងតាម Slider
...
-- ពេលចម្រៀងបញ្ចប់ (ដំណើរការបន្ទាប់ដោយស្វ័យប្រវត្តិ)
Music.Ended:Connect(function()
	...
	if isShuffle then
		...
		local nextIdx
		repeat
			nextIdx = math.random(1, #SongLibrary)
		until not table.find(playedSongs, nextIdx)
...
-- ចាប់ផ្តើម Playlist ដំបូង
updatePlaylist()

-- បិទ/បើក GUI ដោយចុចគ្រាប់ចុច RightControl
...
print("✅ Boombox System Loaded! ចុច RightControl ដើម្បីបង្ហាញ/លាក់ GUI")