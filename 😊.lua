...
local function updateWalkSpeed()
    ...
    if not character then return end
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if humanoid then humanoid.WalkSpeed = math.max(16, 16 * Settings.SpeedBoostMultiplier) end
end
...
local function createNPCESP(npc)
    ...
    State.ESPObjects[npc] = {
        Highlight = highlight, Billboard = billboard, NameLabel = nameLabel,
        HealthBG = healthBG, HealthFill = healthFill, InfoLabel = infoLabel,
        Humanoid = hum, Root = root
    }
end
...
local function scanNPCs()
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and isValidNPC(obj) then createNPCESP(obj) end
    end
end
...
local function createUI(imageAsset)
    ...
    local function notify(titleText, messageText, duration, notificationType)
        ...
        local function remove()
            ...
            tween:Play()
            tween.Completed:Connect(function() if card and card.Parent then card:Destroy() end end)
        end
    ...
    local function addToggle(container, text, default, callback)
        ...
        local function refresh()
            TweenService:Create(switch, TweenInfo.new(0.18), { BackgroundColor3 = state and Theme.Accent or Theme.Surface2 }):Play()
            TweenService:Create(knob, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Position = state and UDim2.new(1, -18, 0.5, -8) or UDim2.fromOffset(2, 2) }):Play()
        end
    ...
    local function addButton(container, text, callback)
        ...
        button.MouseButton1Click:Connect(function() playBeep() pcall(callback) end)
        return button
    end
    ...
    local function closeUI()
        ...
        tween:Play()
        TweenService:Create(toggleBtn, TweenInfo.new(0.22), { Rotation = 0 }):Play()
